...
class AFGN(nn.Module):
    def __init__(self):
        super().__init__()
        self.feat1 = AttTrans1d(5, 32, kernel_size=(5,11), feat_len=25, stride=1, padding=(0,5))
        self.acvt1 = nn.Sequential(nn.BatchNorm2d(32), nn.MaxPool2d((1,2)), nn.ReLU())
        self.feat2 = AttTrans1d(32, 32, kernel_size=(1,5), feat_len=12, stride=1, padding=(0,2))
...
class AttTrans1d(nn.Module):
    '''
    Temporal Attention Feature Transforming Layer for multi-channel 1D features.
    '''
    def __init__(self, in_channels, out_channels, kernel_size, feat_len, stride=1, padding=0, alpha=0.2):
        super().__init__()
